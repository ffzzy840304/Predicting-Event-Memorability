...
class AMNet:
    ...
    def init(self, hps):
        ...
        if hps.front_end_cnn == 'ResNet50FT':
            ...
            model = AMemNetModel(core_cnn, hps, a_res=14, a_vec_size=1024,num_features=27)
    ...
    def get_losses(self, output, outputs, alphas, target, criterion):
        ...
        if output is not None:
           ...
           reg_loss = self.ordinal_loss(output, target)
        ...
        a = outputs.float() * self.hps.mem_loc_w
        a = a ** 2
    ...
    def ordinal_loss(self, y_pred, y_true): 
       _,P=torch.max(y_pred,dim=1) 
       _,T= torch.max(y_true.unsqueeze(1),dim=0) 
       #print(y_pred,y_true)
       diff=(torch.abs(T-P)).float()/(y_pred.size()[0]-1)
       weights=diff
    ...
    def train_epoch(self, epoch, train_loader):
        ...
        for batch_idx, (data, fea, target, _) in enumerate(train_loader):

            for val in target:
                targets.append(val)

            if params.use_cuda:
                data, fea,target = data.cuda(), fea.cuda(), target.long().cuda()
           # print(data.shape)
            data, fea, target = Variable(data), Variable(fea), Variable(target)

            output, outputs, alphas = self.model(data,fea)
        ...
        print("{:03d}:{:06.3f} - {}  [{:.3f} sec]".format(
            took_total_min, took_total, epoch, took_epoch))
    ...
    def train(self):
        ...
        for epoch in range(self.hps.epoch_start + 1, self.hps.epoch_start + self.hps.epoch_max):
            ...
            test_loss = self.eval_model(test_data_loader)
...
# Evaluation
    def eval_model(self, test_loader):
        ...
        for data, features, target, _ in test_loader:
            ...
            target = target.float()
            if self.hps.use_cuda:
                data, features, target = data.cuda(), features.cuda(), target.long().cuda()
            ...
            output, outputs, alphas = self.model(data,features)
        ...
        print('\nValidation: avg_loss: {:.4f} ({:.4f},  {:.4f})   image_inference: {:.3f} us\n'.format(
                test_loss, test_reg_loss, test_att_loss, img_inference_took_avg*1000000.0))
    ...
    def predict(self, test_loader):
        ...
        for data, features,target, names in test_loader:
            ...
            target = target.float()
            if self.hps.use_cuda:
                data, features, target = data.cuda(), features.cuda(), target.cuda()

            data, features, target = Variable(data, volatile=True),Variable(features), Variable(target)

            # print(batches, len(predictions))
            batch_inference_start = time.time()
            output_, outputs_, alphas_ = self.model(data,features)
            batch_inference_took = time.time() - batch_inference_start
            img_inference_took += batch_inference_took / data.size(0)
            batches += 1

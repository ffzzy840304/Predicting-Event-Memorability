...
for i in range(1,6):
  ...
  TRAIN_SPLIT='train_'+str(i)
  VAL_SPLIT='val_'+str(i)
  SAVE_SPLIT='split_'+str(i)
  ...
  for epoch in range(0,40):
    ...
    for batch_idx, (data, fea, target, _) in enumerate(train_data_loader):
        ...
        optimizer.zero_grad()
        output=model(data,fea)
        #print(output,target.unsqueeze(1))
        train_loss = ordinal_loss(output, target.long())
    ...
    with torch.no_grad():              
        ...
        for batch_idx, (data, fea, target, _) in enumerate(test_data_loader):
           ...
           output=model(data,fea)
           #print(output,target)
           val_loss = ordinal_loss(output, target.long())
    ...
    if total_train_loss<train_loss_min:
       ...
       torch.save(model.state_dict(), 'fusion_models/best_fusion_model_'+SAVE_SPLIT+'.pt')

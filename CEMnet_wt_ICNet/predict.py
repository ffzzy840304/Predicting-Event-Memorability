...
def predict(model, test_loader,memory_save_dir):
    ...
    for data, features,target, names in test_loader:
        ...
        fsave.close() 
    return img_names[0], np.argmax(output[m].cpu().data.numpy())
...
for i in range(1,6):
  model=Net(25)
  model.load_state_dict(torch.load('fusion_models/no_encode_distinctive/best_fusion_model_split_'+str(i)+'.pt'))
  model.to(device)

  image_dir='photo_subs_split'+str(i)
  memory_save_dir='memory_icnet_split'+str(i)+'.txt'
  f_list=[]
